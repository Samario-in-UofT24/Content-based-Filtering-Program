...
class GameGraph:
    ...
    def recommend_games(self, liked_game: str, top_k: int = 5) -> list[str]:
        ...
        return [g for g, _ in sorted_games[:top_k]]
...
class SimpleGameGraph:
    """A graph with only recommended games and their visual layout using Plotly."""
    ...
    def add_game_node(self, game: str, score: float, highlight: bool = False, genres: list[str] | None = None) -> None:
        """
        Add a game node to the graph.
        """
        if genres is None:
            genres = []
        self.graph.add_node(game, score=score, highlight=highlight, genres=genres)

    def add_edge(self, game1: str, game2: str, weight: float) -> None:
        """
        Add edge between game nodes.
        """
    ...
    def get_figure(self) -> go.Figure:
        ...
        """
        pos = nx.spring_layout(self.graph, seed=42)  # Use shell_layout or manual pos if needed
        ...
        for node in self.graph.nodes():
            ...
            node_text.append(f'{node}<br>Score: {score:.2f}<br>Genres: {genre_str}')
        ...
        node_trace = go.Scatter(
            ...
            marker={"color": node_color, "size": node_size, "line_width": 2}
        )
...
def build_recommendation_graph(liked_game: str,
                               ...
                               top_k_games: list[str],
                               genre_map: dict[str, list[str]]) -> SimpleGameGraph:
    """Construct a simplified game graph with recommendation result + genre info."""
    graph = SimpleGameGraph()

    liked_game_genres = genre_map.get(liked_game, [])
    graph.add_game_node(liked_game, score=0.0, highlight=True, genres=liked_game_genres)

    for game in top_k_games:
        graph.add_game_node(game, score=scores[game], genres=genre_map.get(game, []))
...
if __name__ == "__main__":
    ...
    python_ta.check_all(config={
        'extra-imports': [
            '__future__', 'collections', 'json', 'math',
            'networkx', 'plotly', 'typing', 'vaderSentiment'
        ],
        'allowed-io': [],
        'max-line-length': 120,
        'disable': ['R1705', 'E9998', 'E9999', 'R0914', 'C9104', 'R0913']
    })
